...
def process_img(file_name,intermediate_file_name, model):
    im = imageio.imread(file_name)  # imagio read might need to be modified
    ...
    plt.imshow(im)
    plt.savefig(intermediate_file_name)
    plt.clf()
    plt.imshow(rgb / 255.)
    plt.savefig(intermediate_file_name)
    return new_rm_ind
